...
export const FileUpload = {
	...
	uploadsOnValidate(file) {
		...
		s.metadata(
			Meteor.bindEnvironment((err, metadata) => {
				...
				const rotated =					typeof metadata.orientation !== 'undefined'
	...
	requestCanAccessFiles({ headers = {}, query = {} }) {
		...
		const isAuthorizedByHeaders =			headers['x-user-id']
			&& headers['x-auth-token']
			&& Users.findOneByIdAndLoginToken(
				headers['x-user-id'],
				headers['x-auth-token'],
			);
		const isAuthorizedByRoom =			rc_room_type
			&& roomTypes
				.getConfig(rc_room_type)
				.canAccessUploadedFile({ rc_uid, rc_rid, rc_token });
		const isAuthorizedByJWT =			settings.get('FileUpload_Enable_json_web_token_for_files')
			&& token
			&& isValidJWT(
				token,
